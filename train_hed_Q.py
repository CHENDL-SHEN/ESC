...
def main(args):
    ...
    train_domain = 'train_aug2' if args.dataset == 'voc12' else 'train'
    ...
    def evaluate(loader):
        ...
        with torch.no_grad():
            ...
            for step, (images, image_ids, tags, gt_masks) in enumerate(loader):
                ...
                inuptfeats = F.interpolate(inuptfeats.float(), size=(
                    12, 12), mode='bilinear', align_corners=False)
                inuptfeats = F.interpolate(inuptfeats.float(), size=(
                    w, h), mode='bilinear', align_corners=False)
                prob = model(images)
                inuptfeats, affmat = refine_with_q(
                    inuptfeats, prob, 20, with_aff=True)
    ...
    for iteration in range(max_iteration):
        ...
        sailencys = sailencys.cuda().view(
            sailencys.shape[0], 1, sailencys.shape[1], sailencys.shape[2])
        # sailencys = (sailencys > 0.2).long()

        # label_1hot = label2one_hot_torch(labels, C=2)  # set C=50 as SSN does
        # label_1hot=sailencys.max()
        sailencys[sailencys>40]=0
        label_1hot = label2one_hot_torch(sailencys, C=40) # set C=50 as SSN does
