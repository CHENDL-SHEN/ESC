...
if __name__ == '__main__':
    try:
        ...
        save_path="/media/ders/zhangyumin/PuzzleCAM/experiments/res/train_peslabels"
        Q_model = fcnmodel.SpixelNet1l_bn().cuda()
        ...
        for filename in train_list:
            ...
            cam0 = cv.imread(os.path.join(cam_path,img_id+str(0)+ '.png'))
            cam1 = cv.imread(os.path.join(cam_path,img_id+str(1)+ '.png'))
            cam2 = cv.imread(os.path.join(cam_path,img_id+str(2)+ '.png'))
            H,W,_=shape(image)
            ...
            resize_in_C=transforms.Resize([H_//16,W_//16])      #H换成了IMAGE的HW
            ...
            cam0=torch.FloatTensor(np.array(cam0))
            cam0=cam0.transpose(2,0).transpose(1,2).unsqueeze(0)
            cam0=resize_in_C(cam0)

            cam1=torch.FloatTensor(np.array(cam1))
            cam1=cam1.transpose(2,0).transpose(1,2).unsqueeze(0)
            cam1=resize_in_C(cam1)

            cam2=torch.FloatTensor(np.array(cam2))
            cam2=cam2.transpose(2,0).transpose(1,2).unsqueeze(0)
            cam2=resize_in_C(cam2)


            prob = Q_model(image.cuda())
            #sp_image= poolfeat(cam.cuda(),prob)
            sp_image0 = upfeat(cam0.cuda(),prob)
            sp_image0=F.interpolate(sp_image0, size=(H, W),mode='nearest')
            #sp_image=resize_out(sp_image)
            sp_image0=sp_image0.squeeze(0).transpose(0,1).transpose(1,2)
            
            sp_image0=sp_image0.cpu()
            sp_image0=sp_image0.detach().numpy()
            cv.imwrite(os.path.join(save_path, img_id + str(0)+'.png'), sp_image0)

            sp_image0 = upfeat(cam1.cuda(),prob)
            sp_image0=F.interpolate(sp_image0, size=(H, W),mode='nearest')
            #sp_image=resize_out(sp_image)
            sp_image0=sp_image0.squeeze(0).transpose(0,1).transpose(1,2)
            ...
            sp_image0=sp_image0.cpu()
            sp_image0=sp_image0.detach().numpy()
            cv.imwrite(os.path.join(save_path, img_id + str(1)+'.png'), sp_image0)

            sp_image0 = upfeat(cam2.cuda(),prob)
            sp_image0=F.interpolate(sp_image0, size=(H, W),mode='nearest')
            #sp_image=resize_out(sp_image)
            sp_image0=sp_image0.squeeze(0).transpose(0,1).transpose(1,2)
            ...
            sp_image0=sp_image0.cpu()
            sp_image0=sp_image0.detach().numpy()
            cv.imwrite(os.path.join(save_path, img_id + str(2)+'.png'), sp_image0)
