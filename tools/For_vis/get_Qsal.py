...
if __name__ == '__main__':
    try:
        ...
        #refinecam= upfeat(refinecam,prob)
        path = '/media/ders/zhangyumin/DATASETS/dataset/newvoc/VOCdevkit/VOC2012/JPEGImages/'
        sal_path = "/media/ders/zhangyumin/PuzzleCAM/experiments/res/train_eps_cam/"
        path_list = os.listdir(path)
        save_path="/media/ders/zhangyumin/PuzzleCAM/experiments/res/train_Q_spsal"
        Q_model = fcnmodel.SpixelNet1l_bn().cuda()
        ...
        for filename in train_list:
            ...
            img_id=img_id.split('\n')[0]
            image = cv.imread(os.path.join(path,img_id+ '.jpg'))
            sal = cv.imread(os.path.join(sal_path,img_id+ '.png'))
            ...
            sal=torch.FloatTensor(np.array(sal))
            sal=sal.transpose(2,0).transpose(1,2).unsqueeze(0)
            sal=resize_in(sal)
            #image.cuda()
            #image=image.float
            ...
            sp_image= poolfeat(sal.cuda(),prob)
