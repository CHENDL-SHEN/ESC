...
def get_params():
    ...
    parser.add_argument(
        '--data_dir', default='VOC2012/VOCdevkit/VOC2012/', type=str)
    ...
    parser.add_argument('--lr', default=0.2, type=float)  # ***********调
    ...
    parser.add_argument('--ksalq', default=0, type=float)
    ...
    parser.add_argument(
        '--Qmodelpath', default='experiments/models/modelbest18.pth', type=str)  # ***********改
    parser.add_argument('--Qloss_rtime', default=1, type=int)
    parser.add_argument('--topkt', default=0.2, type=float)
...
class SetLoss(_Loss):
    ...
    def forward(self, logits, prob, sailencys, labels):
        sailencys=sailencys/255.0

        # sailencys[sailencys==255]=0

        # sailencys=label2one_hot_torch(sailencys,21)
        # sailencys=1-sailencys[:,0:1]
        if(self.args['withQ']):
        ...
        loss_cls = F.multilabel_soft_margin_loss(
            tagpred[:, 1:].view(tagpred.size(0), -1), labels[:, 1:])
        if(True):
            cam = logits
            sailencys = F.interpolate(sailencys.float(), size=(h, w))

            label_map = labels[:, 1:].view(b, 20, 1, 1).expand(
                size=(b, 20, h, w)).bool()  # label_map_bg[0,:,0,0]
            # Map selection
            ...
            iou_saliency = (torch.round(sal_pred[:, 1:].detach()) * torch.round(sailencys)).view(b, 20, -1).sum(-1) / \
                (torch.round(sal_pred[:, 1:].detach()) +
                 1e-04).view(b, 20, -1).sum(-1)

            valid_channel = (iou_saliency > self.args["tao"]).view(
                b, 20, 1, 1).expand(size=(b, 20, h, w))
            ...
            # label_map_fg[1,:,0,0] torch.sum(fg_map[7][0]>0.5) F.mse_loss(2*fg_map,sailencys)
        ...
        if(self.args['withQ']):
            ...
            # reconstr_feat5.reshape(b,5,-1).max(dim=2)
            reconstr_feat5 = torch.zeros(b, 5, h, w).float()
            reconstr_feat5=logits
            for ii in range(0):
                cur = 0
                for jj in range(21):
                    if(labels[ii, jj] == 1):
            ...
            refinecam, affmat = refine_with_q(
                refinecam, prob, self.args['Qloss_rtime'], with_aff=True)
            tensor_ecr1=torch.pow(reconstr_feat-refinecam,2)
            q_loss=torch.mean(torch.topk(tensor_ecr1.view(b,-1), k=(int)(2*h*w*self.args['topkt']), dim=-1)[0])
...
def main(args):
    ...
    ###################################################################################
    if(args['withQ']):
        evaluatorA = evaluator.evaluator(domain='train', fast_eval=False,savepng=True)
    else:
    ...
    train_transforms = [
        RandomResize_For_Segmentation(
            args["min_image_size"], args["max_image_size"]),
    ...
    train_dataset = VOC_Dataset_For_MNSS(args["data_dir"], 'VOC2012/VOCdevkit/VOC2012/SegmentationClassAug/', 'train_aug', train_transform)
    # train_dataset = VOC_Dataset_For_MNSS(args["data_dir"], 'VOC2012/VOCdevkit/VOC2012/saliency_map/', 'train_aug', train_transform)

    # valid_dataset = VOC_Dataset_For_Segmentation(args["data_dir, 'train', test_transform)
    valid_dataset = VOC_Dataset_For_Testing_CAM(
    ...
    valid_loader = DataLoader(
        valid_dataset, batch_size=args["batch_size"], num_workers=1, shuffle=False, drop_last=True)
    ...
    if args["architecture"] == 'DeepLabv3+':
        model = DeepLabv3_Plus(args["backbone"], num_classes=meta_dic['classes'] +
                               1, mode=args["mode"], use_group_norm=args["use_gn"])
    elif args["architecture"] == 'Seg_Model':
        model = Seg_Model(args["backbone"],
                          num_classes=meta_dic['classes'] + 1)
    elif args["architecture"] == 'CSeg_Model':
        model = CSeg_Model(
            args["backbone"], num_classes=meta_dic['classes'] + 1)
    ...
    model = model.cuda()
    model.train()
    # model.load_state_dict(torch.load('experiments/models/sanetQ_nest50/2021-10-26 11:58:07.pth'))
    ...
    def save_model_fn_for_backup(): return save_model(model, model_path.replace(
        '.pth', f'_backup.pth'), parallel=the_number_of_gpu > 1)
    ...
    optimizer = PolyOptimizer(
        params, lr=args["lr"], momentum=0.9, weight_decay=args["wd"], max_step=max_iteration, nesterov=args["nesterov"])
    ...
    for iteration in range(max_iteration):
        ...
        sailencys = sailencys.cuda().view(
            sailencys.shape[0], 1, sailencys.shape[1], sailencys.shape[2])#sailencys[1].cpu().numpy()
        prob = None
        if(args['withQ']):
            prob = Q_model(images)
