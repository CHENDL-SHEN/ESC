...
path = '/media/ders/zhangyumin/PuzzleCAM/experiments/res/SCAMS/test/QCAM32_50'
path_list = os.listdir(path)
save_path="/media/ders/zhangyumin/PuzzleCAM/experiments/res/train_pse"
...
for filename in path_list:
    ...
    for i in range(21):
        ...
        for j in range(3):
            ...
            #npy_=np.squeeze(npy[i,:,:])
            npy_=npy[i,:,:]
            npy_fg=(npy_>0.05).astype(np.int_)
            result_cam[:,:,j]=(npy_fg*bgr_img[:,:,j])
