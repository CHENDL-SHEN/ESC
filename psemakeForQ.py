...
class evaluator:
    def __init__(self,domain='train',savept=False,savepng=False,th_bg=[0.1],th_step=[0.4],th_fg=[0.2]) -> None:
        self.C_model = None
        self.Q_model = None
        ...
        for bt in self.th_bg:
            for ts in self.th_step:
                for tf in  self.th_fg:
                    self.config_list.append((bt,ts,tf))

        # self.th_list = [0.3]
        # self.refine_list = [20]

        self.meterlist=[ Calculator_For_mIoU('./data/VOC_2012.json') for x in self.config_list]
        ...
        self.save_path='experiments/res/psefortrainQ/'
    ...
    def getbest_miou(self,clear=True):
        best_iou=0
        best_parm=None
        for parm,meter in zip(self.config_list,self.meterlist):
            ...
            with torch.no_grad():
                ...
                for step, (images,image_ids, tags, gt_masks) in enumerate( self.valid_loader ):
                    ...
                    for cfg in self.config_list:
                        ...
                        cams_bg=cams.clone()
                        cams_fg=cams.clone()
                        cams_bg[:,0]=cfg[0]#predictions.max()
                        cams_fg[:,0]=cfg[0]+cfg[1]#predictions.max()

                        predictions1=torch.argmax(cams_bg,dim=1)
                        predictions2=torch.argmax(cams_fg,dim=1)
                        fgsort = torch.sort(cams[:,1:],1,True)[0]
                        ignore_masks = predictions1 != predictions2#fgsort[0][0][:,0,0]
                        ignore_masks |= (cfg[2]*fgsort[:,0]<fgsort[:,1])&(predictions1>0)#
                        predictions=predictions1.clone()
                        predictions[ignore_masks] =255
                        gt_masks[ignore_masks]=255
                        for batch_index in range(images.size()[0]):
                            pred_mask = get_numpy_from_tensor(predictions[batch_index])
                            gt_mask = get_numpy_from_tensor(gt_masks[batch_index])
                            gt_mask=cv2.resize(gt_mask,(pred_mask.shape[1],pred_mask.shape[0]), interpolation=cv2.INTER_NEAREST)
                            self.meterlist[self.config_list.index(cfg)].add(pred_mask, gt_mask)#self.getbest_miou(clear=False)
                            if(self.savepng):
                                if(self.C_model!=None):
                                    save_dir = self.save_path+str(cfg)
                                    if not os.path.exists( save_dir):
                                        os.mkdir(save_dir)
                                    img_path=os.path.join(save_dir,image_ids[batch_index]+'.png')
