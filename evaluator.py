...
class evaluator:
    def __init__(self, domain='train', withQ=True, save_np_path=None,savepng_path=None,muti_scale=False,th_list=list(np.arange(0.2, 0.5, 0.1)),refine_list = range(0, 50, 10)) -> None:
        self.C_model = None
        self.Q_model = None
        self.with_Q = withQ
        if(muti_scale):
        ...
        self.meterlist = [Calculator_For_mIoU(
            './data/VOC_2012.json') for x in self.parms]
        ...
        valid_dataset = Dataset_For_Evaluation(
            'VOC2012/VOCdevkit/VOC2012/', self.domain, test_transform)
...
if __name__ =="__main__":
    ...
    # model = SP_CAM_Model('resnest50', num_classes=20 + 1,process=32,fgORall=True)
    model = model.cuda()
    model.train()
    model.load_state_dict(torch.load(
        '/media/ders/zhangyumin/PuzzleCAM/finalmodel/base_sal/our-resnest50/2021-11-12 15:09:03.pth'))
    Q_model = fcnmodel.SpixelNet1l_bn().cuda()
    Q_model.load_state_dict(torch.load('experiments/models/bestQ.pth'))
    Q_model = nn.DataParallel(Q_model)
    Q_model.eval()
    evaluatorA = evaluator(domain='train_aug',savepng_path="finalmodel/base_sal/our-resnest50/pseudo_new_aug", withQ=True,refine_list=[4],th_list=[0.3])
