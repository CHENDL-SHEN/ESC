...
predict_folder="/media/ders/zhangyumin/irn-master/result/ir_label_spcam22_pcm/"
# predict_folder ="/media/ders/zhangyumin/PuzzleCAM/experiments/predictions/area66_train_exp_b05@train@scale=0.5,1.0,1.5,2.0/"
...
def compare(start,step,TP,P,T, name_list):
    for idx in range(start,len(name_list),step):
        ...
        if os.path.isfile(predict_folder + name + '.npy'):
            ...
            if 'cams' in predict_dict.keys():
                cams = predict_dict['cams']
...
def do_python_eval(predict_folder, gt_folder, name_list, num_cls=21, num_cores=8):
    ...
    # sum([(TP[i].value) for i in range(1,21)])
...
if __name__ == '__main__':
    ...
    if args.mode == 'png':
        ...
        if args.threshold is None:
            th_list = np.arange(0.15, 0.65, 0.05).tolist()
