...
from chainer import cross_entropy
...
def get_params():
    ...
    parser.add_argument('--max_image_size', default=560, type=int)
    ###############################################################################
    # Network
    ###############################################################################
    ...
    parser.add_argument('--tag', default='train_sp_cam_VOC_ppl', type=str)
...
def main(args):
    ...
    # saliency_dir="/media/ders/zhangyumin/PuzzleCAM/VOC2012/VOCdevkit/VOC2012/SegmentationClassAug/"
    ...
    best_valid_mIoU =-1
    class_loss_fn2 = nn.CrossEntropyLoss(ignore_index=255).cuda()
    ...
    for iteration in range(max_iteration):
        ...
        sailencys = sailencys.cuda()
        # sailencys = sailencys.cuda().view(sailencys.shape[0],sailencys.shape[1],sailencys.shape[2])
        prob=None
        ...
        label=sailencys.clone()
        label[(label>20)*(label<255)]=0#((label>20)*(label<255)).sum()
        sailencys[sailencys>20]=0
        # f_c=F.one_hot(sailencys.long(),21).view(b,-1,21).transpose(1,2).view(b,21,h*16,w*16)
        # f_c_min=poolfeat(f_c,prob)
        # f_c_min=torch.argmax(f_c_min,dim=1)
        # f_c_min[f_c_min==21]=255
        
        b, c, h, w = logits.size()
        tagpred = logitsmin#
        logits_up=upfeat(logits,prob.detach()) 
        # cls_loss = F.multilabel_soft_margin_loss(tagpred[:, 1:].view(tagpred.size(0), -1), labels[:,1:])
        cls_loss=class_loss_fn2(logits_up,label.long().cuda())#(sailencys==255).sum()#sailencys[0,-1,-1]
        ...
        # target_feat_tile=tile_features(target_feat,1)
        # fg_cam_tile=tile_features(fg_cam,1)
        
        # sal_loss=lossfn(fg_cam_tile,target_feat_tile).mean()

        alpha=0
        sal_loss=cls_loss
        loss= cls_loss+alpha*(sal_loss) 
